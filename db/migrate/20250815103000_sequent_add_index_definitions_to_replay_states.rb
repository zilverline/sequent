  ...
  def change
    Sequent::Support::Database.with_search_path(Sequent.configuration.event_store_schema_name) do
      ...
      add_check_constraint :replay_states, <<~SQL, name: 'valid_replay_state'
        state IN ('created', 'prepared', 'replaying', 'catching_up', 'replayed', 'optimized', 'live', 'failed', 'aborted')
      SQL
      ...
      SQL
      execute <<~SQL
        CREATE UNIQUE INDEX replay_states_active_replay_idx ON replay_states ((TRUE)) WHERE state NOT IN ('live', 'aborted')
      SQL
    end
  end
end
