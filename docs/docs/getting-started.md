...
- `spec/post/command_spec.rb`: Here we test that when a command is given, certain events will occur.
- `spec/post/projector_spec.rb`: Here we test that when an event occurs, the projector updates the view records.
